...
| **`@helm/domain`** — pure evaluation cores: **KPI** (attainment, direction-aware RAG, scorecard rollups, forecast), **Risk** (likelihood×impact scoring, severity bands, appetite breach, heatmap, register aggregation), **Portfolio** (value/risk quadrants, prioritization, rollups). | **56/56 unit tests pass** (`packages/domain`). |
| **`@helm/api`** — NestJS backend (Phase 1): JWT auth + RBAC/ABAC guards; **KPI, Risk/KRI and Portfolio** modules end-to-end (CRUD, scorecard, heatmap, matrix); AI endpoint wired to `@helm/ai-core`; Prisma repos + seed. Plus a **zero-dependency reference server** mirroring the endpoints. | Reference server exercised end-to-end; prototype KPI, Risk and Portfolio views read **live data** from it (verified in-browser). |
...
Risk & KRIs, and Portfolio views flip their badge to “Live API”:
