...
export class KpiRepository {
  ...
  findMany(tenantId: string, where: Prisma.KpiWhereInput) {
    return this.prisma.db.kpi.findMany({
  ...
  findOne(tenantId: string, id: string) {
    return this.prisma.db.kpi.findFirst({
  ...
  create(tenantId: string, data: Prisma.KpiCreateInput) {
    return this.prisma.db.kpi.create({ data: { ...data, tenant: { connect: { id: tenantId } } } });
  }

  update(tenantId: string, id: string, data: Prisma.KpiUpdateInput) {
    // updateMany enforces the tenant guard in the WHERE clause.
    return this.prisma.db.kpi.updateMany({ where: { tenantId, id, deletedAt: null }, data });
  }

  softDelete(tenantId: string, id: string) {
    return this.prisma.db.kpi.updateMany({
      where: { tenantId, id, deletedAt: null },
  ...
  addMeasurement(kpiId: string, data: Prisma.KpiMeasurementUncheckedCreateInput) {
    return this.prisma.db.kpiMeasurement.upsert({
